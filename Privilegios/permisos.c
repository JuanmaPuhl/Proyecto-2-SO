...
int validarParametros(int argc){
    ...
    Solo se necesita 1 para la direccion y 1 para el modo.
    Se cuenta 1 extra por la llamada al programa.*/
    if(argc>3){
...
/**Metodo auxiliar para mostrar por pantalla que permisos se le asignaron al
archivo ingresado.*/
void mostrarCambios(char modo){
    if(modo=='0')
        printf("Ningun permiso.\n");
    if(modo=='1')
        printf("Ejecucion.\n");
    if(modo=='2')
        printf("Escritura.\n");
    if(modo=='3')
        printf("Escritura y Ejecucion.\n");
    if(modo=='4')
        printf("Lectura.\n");
    if(modo=='5')
        printf("Lectura y Ejecucion.\n");
    if(modo=='6')
        printf("Lectura y Escritura.\n");
    if(modo=='7')
        printf("Lectura, Escritura y Ejecucion.\n");
}
...
int main(int argc, char* argv[]){
    ...
    if(resultado<0){ //Hubo error
        ...
        printf("%sSe actualizaron los permisos con exito.\n",VERDE);
        printf("Se asignaron los siguientes permisos: \n");
        printf("Propietario: ");
        mostrarCambios(modo[0]);
        printf("Grupo: ");
        mostrarCambios(modo[1]);
        printf("Otros: ");
        mostrarCambios(modo[2]);
        printf("%s",CERRAR);
